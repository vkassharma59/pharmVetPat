...
export class ChemiTrackerComponent {
  ...
  private async createExcelWithHeader(data: any[], titleKeyword: string): Promise<Blob> {
    ...
    const title = `CHEMI TRACKER DATA REPORT`;
  ...
  downloadExcel(): void {
    ...
    this._currentChildAPIBody = { ...this.chemiAPIBody, filters:{...this.chemiAPIBody.filters} };
    ...
    this.mainSearchService.chemiTrackerdownloadexcel(this._currentChildAPIBody).subscribe({
      next: async(res:Blob)=>{
        try{
          ...
          a.href=url;
          a.download='Chemi-Tracker-Report.xlsx';
          a.click();
