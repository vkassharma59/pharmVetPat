...
export class CanadaComponent implements OnInit {
 ...
 @Input()
  ...
  private async createExcelWithHeader(data: any[], titleKeyword: string): Promise<Blob> {
    ...
    const worksheet = workbook.addWorksheet('Health Canada');
    ...
    const title = `Health Canada Report`;
  ...
  downloadExcel(): void {
    ...
    this._currentChildAPIBody = { ...this.canadaPatentApiBody, filters:{...this.canadaPatentApiBody.filters} };
