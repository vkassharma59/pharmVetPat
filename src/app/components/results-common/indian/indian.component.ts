...
export class IndianComponent {
  ...
  indianApiBody: any;
    ...
    private async createExcelWithHeader(data: any[], titleKeyword: string): Promise<Blob> {
      ...
      const worksheet = workbook.addWorksheet('Indian');
      ...
      const title = `Indian Medicine Report`;
    ...
    downloadExcel(): void {
      ...
      this.mainsearchsearvice.indianMedicinedownloadexcel(this._currentChildAPIBody).subscribe({
        next: async (res: Blob) => {
          try {
            if (!res || res.size === 0) {
    ...
    //   this._currentChildAPIBody = {
    //     search_type: this.indianApiBody?.search_type || 'TRRN', // or use dynamic if needed
    //     keyword:
    //       this.keyword?.trim() ||
    //       this.currentChildAPIBody?.keyword?.trim() ||
    //       this.indianApiBody?.keyword?.trim() ||
    //       '',
    //     filters: this.indianApiBody?.filters || {},
    //   };
    ...
    //     XLSX.writeFile(workbook, 'Indian Medicine.xlsx');
